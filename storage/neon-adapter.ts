...
import { _sql } from "@neondatabase/serverless"
import {
  User,
  Project,
  Competitor,
...
export class NeonStorageAdapter implements StorageAdapter {
  ...
  async createProject(userId: number, name: string): Promise<Project> {
    ...
    const result = await this.client.query(
      'INSERT INTO "Projects" (user_id, name, is_active) VALUES ($1, $2, true) RETURNING *',
      [userId, name]
    )
  ...
  async getCompetitorAccounts(projectId: number): Promise<Competitor[]> {
    ...
    const result = await this.client.query(
      'SELECT * FROM "Competitors" WHERE project_id = $1 ORDER BY created_at DESC',
      [projectId]
    )
  ...
  async createCompetitor(
    ...
    const result = await this.client.query(
      'INSERT INTO "Competitors" (project_id, username, instagram_url, notes) VALUES ($1, $2, $3, $4) RETURNING *',
      [projectId, username, instagramUrl, notes]
    )
  ...
  async updateCompetitor(
    ...
    const result = await this.client.query(
      `UPDATE "Competitors" SET ${updateFields.join(
        ", "
  ...
  async getTrackingHashtags(projectId: number): Promise<Hashtag[]> {
    ...
    const result = await this.client.query(
      'SELECT * FROM "Hashtags" WHERE project_id = $1 ORDER BY created_at DESC',
      [projectId]
    )
  ...
  async createHashtag(
    ...
    const result = await this.client.query(
      'INSERT INTO "Hashtags" (project_id, name, notes) VALUES ($1, $2, $3) RETURNING *',
      [projectId, cleanName, notes]
    )
  ...
  async updateHashtag(
    ...
    const result = await this.client.query(
      `UPDATE "Hashtags" SET ${updateFields.join(", ")} WHERE id = $${valueIndex} RETURNING *`,
      values
    )
  ...
  async deleteHashtag(hashtagId: number): Promise<boolean> {
    ...
    const result = await this.client.query(
      'DELETE FROM "Hashtags" WHERE id = $1 RETURNING id',
      [hashtagId]
    )
  ...
  async getReels(
    ...
    const countResult = await this.client.query(
      'SELECT COUNT(*) as total FROM "ReelsContent" WHERE project_id = $1',
      [projectId]
    )
    ...
    const result = await this.client.query(
      'SELECT * FROM "ReelsContent" WHERE project_id = $1 ORDER BY publication_date DESC LIMIT $2 OFFSET $3',
    ...
    try {
      ...
      for (const reel of reels) {
        ...
        const existingResult = await this.client.query(
          'SELECT id FROM "ReelsContent" WHERE reels_url = $1',
          [reel.reels_url]
        )
 ...
 * @param connectionString Строка подключения к базе данных Neon
 */
export function createNeonStorageAdapter(
