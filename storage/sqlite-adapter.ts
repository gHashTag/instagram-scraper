 ...
 */
export function createSQLiteStorageAdapter(dbPath?: string): StorageAdapter {
  ...
  return {
      ...
      try {
        ...
        `
          ...
          .get(
            ...
            competitorData.instagramUrl,
            competitorData.notes || null
          )
    ...
    async getCompetitorAccounts(projectId: string): Promise<Competitor[]> {
      try {
        const rows = db
          .prepare(
            `
    ...
    async getTrackingHashtags(projectId: string): Promise<Hashtag[]> {
      try {
        const rows = db
          .prepare(
            `
        ...
        db.transaction(() => {
          for (const reel of reels) {
            const info = insertStmt.run(
              ...
              reel.authorUsername || null,
              reel.authorId || null,
              reel.audioTitle || null,
              reel.audioArtist || null
            )
