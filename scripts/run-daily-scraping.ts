...
// import {
//   initializeNeonStorage, // Закомментировано
//   closeNeonStorage, // Закомментировано
//   getAllActiveUsers, // Закомментировано
//   getProjectsByUserId, // Закомментировано
//   getCompetitorAccounts, // Закомментировано
//   getTrackingHashtags, // Закомментировано
//   scrapeInstagramReels, // Этот импорт есть в agent/index, оттуда и надо брать
//   saveReels, // Закомментировано
//   logParsingRun, // Закомментировано
// } from "../index"

// import type { User, Project, Competitor, Hashtag } from "../index" // Закомментировано, так как типы из ../index (бывший @/types) удалены
...
// const MIN_VIEWS = parseInt(process.env.MIN_VIEWS || "50000"); // TS6133: 'MIN_VIEWS' is declared but its value is never read.
// const MAX_AGE_DAYS = parseInt(process.env.MAX_AGE_DAYS || "14"); // TS6133: 'MAX_AGE_DAYS' is declared but its value is never read.
...
async function scrapeSource(
  _projectId: number, // TS6133: 'projectId' is declared but its value is never read.
  sourceType: "competitor" | "hashtag",
  _sourceId: number, // TS6133: 'sourceId' is declared but its value is never read.
  sourceUrl: string,
  _runId: string // TS6133: 'runId' is declared but its value is never read.
) {
  try {
    console.log(
      `Скрапинг ${sourceType === "competitor" ? "аккаунта" : "хэштега"}: ${sourceUrl}`
    );

    // Создаем запись о начале скрапинга
    // await logParsingRun({ // Временно закомментировано
    //   run_id: runId,
    ...
    // const reels = await scrapeInstagramReels(sourceUrl, { // Временно закомментировано
    //   apifyToken: APIFY_TOKEN,
    //   minViews: MIN_VIEWS,
    //   maxAgeDays: MAX_AGE_DAYS,
    // });
    const reels = [] as any[]; // Временная заглушка

    console.log(`Получено ${reels.length} Reels для ${sourceUrl}`);

    // Сохраняем данные в базу
    // const savedCount = await saveReels(reels, projectId, sourceType, sourceId); // Временно закомментировано
    const savedCount = 0; // Временная заглушка
...
async function runDailyScraping() {
  ...
  try {
    ...
    // const users = await getAllActiveUsers(); // Временно закомментировано
    const users = [] as any[]; // Временная заглушка
    ...
    for (const user of users) {
      ...
      // const projects = await getProjectsByUserId(user.id); // Временно закомментировано
      const projects = [] as any[]; // Временная заглушка
      ...
      for (const project of activeProjects) {
        ...
        // const competitors = await getCompetitorAccounts(project.id, true); // Временно закомментировано
        const competitors = [] as any[]; // Временная заглушка
        console.log(
          `Найдено ${competitors.length} активных аккаунтов конкурентов`
        );
        ...
        for (const competitor of competitors) {
          try {
            ...
            console.error(
              `Ошибка при обработке конкурента ${competitor.instagram_url}:`,
              error
            );
            totalErrors++;
          }
        ...
        // const hashtags = await getTrackingHashtags(project.id, true); // Временно закомментировано
        const hashtags = [] as any[]; // Временная заглушка
        ...
        for (const hashtag of hashtags) {
          try {
            ...
            console.error(
              `Ошибка при обработке хэштега #${hashtag.hashtag}:`,
              error
            );
            totalErrors++;
          }
    ...
    console.error(
      "КРИТИЧЕСКАЯ ОШИБКА при выполнении ежедневного скрапинга:",
      error
    );
  } finally {
