...
export class SqliteAdapter implements StorageAdapter {
  ...
  private ensureConnection(): Database {
    if (!this.db) {
      throw new Error(
        "Нет подключения к SQLite базе данных. Вызовите connect() перед использованием адаптера."
      )
    }
    return this.db
  }
   ...
   * @param projectId ID проекта
   */
  async getCompetitors(projectId: number): Promise<Competitor[]> {
    const db = this.ensureConnection()
   ...
   * @param projectId ID проекта
   */
  async getHashtags(projectId: number): Promise<Hashtag[]> {
    const db = this.ensureConnection()
   ...
   */
  async saveReel(reel: Partial<ReelContent>): Promise<ReelContent> {
    const db = this.ensureConnection()
    ...
    try {
      ...
      if (result.lastInsertRowid) {
        ...
        const savedReel = getReel.get(reelId) as ReelContent
      ...
      if (reel.instagram_id) {
        ...
        const updatedReel = getReel.get(reel.instagram_id) as ReelContent
  ...
  async getReels(
    ...
    try {
      ...
      const reels = statement.all(...params) as ReelContent[]

      return reels
    } catch (error) {
      console.error("Ошибка при получении Reels из SQLite:", error)
      throw new Error(
        `Ошибка при получении Reels: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
