...
import { initializeNeonStorage, closeNeonStorage, createUser, createProject, addCompetitorAccount, addTrackingHashtag, scrapeInstagramReels, mockGetUserByTelegramId, mockGetProjectsByUserId, mockCreateProject, mockGetHashtagsByProjectId, mockGetCompetitorAccounts, mockCreateCompetitor, mockCreateHashtag, mockSaveReels, mockGetReelsByProjectId, } from "../../agent/index";
...
vi.mock("../../storage/neonStorage-multitenant", async () => {
    const actual = await vi.importActual("../../storage/neonStorage-multitenant");
    return {
        ...actual,
...
vi.mock("../../agent/instagram-scraper", () => {
    return {
        scrapeInstagramReels: vi
            .fn()
            .mockImplementation((_instagramUrl, options) => {
            return Promise.resolve([
                {
...
describe("Мультитенантный скрапер Instagram Reels", () => {
    ...
    describe("Управление пользователями и проектами", () => {
        ...
        it("Должен создать проект", async () => {
            const project = await createProject(1, "Test Project", "Тестовый проект", "Эстетическая медицина");
            expect(project).toBeDefined();
            expect(project.name).toBe("Test Project");
        });
        it("Должен добавить аккаунт конкурента", async () => {
            const account = await addCompetitorAccount(1, "https://www.instagram.com/clinicajoelleofficial", "Clinica Joelle");
            expect(account).toBeDefined();
            expect(account.instagram_url).toBe("https://www.instagram.com/clinicajoelleofficial");
        });
    ...
    describe("Скрапинг Instagram Reels", () => {
        ...
        it("Должен обрабатывать ошибки Apify", async () => {
            ...
            await expect(scrapeInstagramReels("clinicajoelleofficial", {
                apifyToken: "mock-apify-token",
...
describe("Scraper Module - Multitenant Storage", () => {
    ...
    describe("getUserByTelegramId", () => {
        it("should return null when user not found", async () => {
            ...
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/SELECT.*FROM\s+users.*WHERE\s+telegram_id\s*=\s*\$1/i), [123456789]);
        });
    ...
    describe("getProjectsByUserId", () => {
        it("should return empty array when no projects found", async () => {
            ...
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/SELECT.*FROM\s+projects.*WHERE\s+user_id\s*=\s*\$1/i), [1]);
        });
    ...
    describe("createProject", () => {
        it("should create a new project", async () => {
            ...
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/INSERT\s+INTO\s+projects.*VALUES\s*\(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*\)/i), [1, "New Project", true]);
        });
    });
    describe("getHashtagsByProjectId", () => {
        it("should return empty array when no hashtags found", async () => {
            ...
            expect(result).toEqual([]);
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/SELECT.*FROM\s+hashtags.*WHERE\s+project_id\s*=\s*\$1/i), [1]);
        });
    ...
    describe("getCompetitorAccounts", () => {
        it("should return empty array when no competitors found", async () => {
            ...
            expect(result).toEqual([]);
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/SELECT.*FROM\s+competitors.*WHERE\s+project_id\s*=\s*\$1/i), [1]);
        });
    ...
    describe("createCompetitor", () => {
        it("should create a new competitor account", async () => {
            ...
            const result = await mockCreateCompetitor(mockNeon, 1, "newcompetitor", "https://instagram.com/newcompetitor");
            expect(result).toEqual(newCompetitor);
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/INSERT\s+INTO\s+competitors.*VALUES\s*\(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*\)/i), [1, "newcompetitor", "https://instagram.com/newcompetitor"]);
        });
    });
    describe("createHashtag", () => {
        it("should create a new hashtag", async () => {
            ...
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/INSERT\s+INTO\s+hashtags.*VALUES\s*\(\s*\$1\s*,\s*\$2\s*\)/i), [1, "newtag"]);
        });
    });
    describe("saveReels", () => {
        it("should save reels in bulk", async () => {
            ...
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/INSERT\s+INTO\s+reels/i), expect.arrayContaining([]));
        });
    });
    describe("getReelsByProjectId", () => {
        it("should return empty array when no reels found", async () => {
            ...
            expect(result).toEqual([]);
            expect(mockNeon.query).toHaveBeenCalledWith(expect.stringMatching(/SELECT.*FROM\s+reels.*WHERE\s+project_id\s*=\s*\$1/i), [1]);
        });
