...
import { createMockContext, createContextWithProjects, } from "../../../__tests__/__mocks__/context-factory";
import { enterHandler, exitSceneHandler, createProjectHandler, backToProjectsHandler, textMessageHandler, } from "../../../__tests__/__mocks__/project-scene-handlers";
...
describe("Project Scene", () => {
    ...
    describe("enter handler", () => {
        it("should show projects list when user has projects", async () => {
            ...
            expect(ctx.reply).toHaveBeenCalledWith("Ваши проекты:", expect.anything());
        ...
        it("should show no projects message when user has no projects", async () => {
            ...
            expect(ctx.storage?.getUserByTelegramId).toHaveBeenCalledWith(123456789);
            expect(ctx.storage?.getProjectsByUserId).toHaveBeenCalledWith(1);
            expect(ctx.reply).toHaveBeenCalledWith("У вас нет проектов. Создайте проект с помощью кнопки ниже.", expect.anything());
            expect(ctx.storage?.close).toHaveBeenCalled();
        });
        it("should handle unauthorized user", async () => {
            ...
            expect(ctx.storage?.initialize).toHaveBeenCalled();
            expect(ctx.storage?.getUserByTelegramId).toHaveBeenCalledWith(123456789);
            expect(ctx.reply).toHaveBeenCalledWith("Вы не зарегистрированы. Пожалуйста, используйте сначала основные команды бота.");
    ...
    describe("action handlers", () => {
        it("should handle exit_scene action", async () => {
            ...
            // Проверяем вызовы функций
            expect(ctx.answerCbQuery).toHaveBeenCalledWith("Выход из режима управления проектами");
            expect(ctx.reply).toHaveBeenCalledWith("Вы вышли из режима управления проектами", expect.objectContaining({
        ...
        it("should handle create_project action", async () => {
            ...
            const ctx = createMockContext();
            // Вызываем обработчик напрямую
            await createProjectHandler(ctx);
            // Проверяем вызовы функций
            expect(ctx.answerCbQuery).toHaveBeenCalledWith();
            expect(ctx.reply).toHaveBeenCalledWith("Введите название нового проекта:", expect.anything());
    ...
    describe("text message handler", () => {
        it("should handle unknown text message", async () => {
            ...
            expect(ctx.reply).toHaveBeenCalledWith("Я не понимаю эту команду. Используйте кнопки для управления проектами.");
        });
        ...
        it.skip("should handle project creation", async () => {
            ...
            expect(ctx.reply).toHaveBeenCalledWith('Проект "Test Project" успешно создан!', expect.anything());
