...
export function createMockContext(options = {}) {
    ...
    const mockInitialize = options.storageOptions?.initialize || vi.fn().mockResolvedValue(undefined);
    const mockClose = options.storageOptions?.close || vi.fn().mockResolvedValue(undefined);
    const mockGetUserByTelegramId = options.storageOptions?.getUserByTelegramId ||
        vi.fn().mockResolvedValue(null);
    const mockGetProjectsByUserId = options.storageOptions?.getProjectsByUserId || vi.fn().mockResolvedValue([]);
    const mockCreateProject = options.storageOptions?.createProject || vi.fn().mockResolvedValue(null);
    const mockGetHashtagsByProjectId = options.storageOptions?.getHashtagsByProjectId ||
        vi.fn().mockResolvedValue([]);
    const mockGetCompetitorsByProjectId = options.storageOptions?.getCompetitorsByProjectId ||
        vi.fn().mockResolvedValue([]);
    const mockGetCompetitorAccounts = options.storageOptions?.getCompetitorAccounts ||
        vi.fn().mockResolvedValue([]);
    const mockCreateCompetitor = options.storageOptions?.createCompetitor || vi.fn().mockResolvedValue(null);
    const mockCreateHashtag = options.storageOptions?.createHashtag || vi.fn().mockResolvedValue(null);
 ...
 */
export function createContextWithUser(userId, telegramId, options = {}) {
 ...
 */
export function createContextWithProjects(userId, telegramId, projects, options = {}) {
