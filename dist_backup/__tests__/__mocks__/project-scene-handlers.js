 ...
 */
export const enterHandler = async (ctx) => {
    try {
        ...
        if (!projects || projects.length === 0) {
            await ctx.reply("У вас нет проектов. Создайте проект с помощью кнопки ниже.", {
 ...
 */
export const exitSceneHandler = async (ctx) => {
 ...
 */
export const createProjectHandler = async (ctx) => {
 ...
 */
export const backToProjectsHandler = async (ctx) => {
    await ctx.answerCbQuery();
...
export const textMessageHandler = async (ctx) => {
    ...
    if (ctx.scene.session.step === ScraperSceneStep.CREATE_PROJECT) {
        ...
        if (projectName.length < 3) {
            await ctx.reply("Название проекта должно содержать не менее 3 символов. Пожалуйста, попробуйте еще раз.");
            return;
        }
        try {
            await ctx.storage?.initialize();
