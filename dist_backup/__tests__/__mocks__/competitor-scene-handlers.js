 ...
 */
export const competitorsProjectHandler = async (ctx, projectId) => {
    try {
        await ctx.storage?.initialize();
 ...
 */
export const addCompetitorHandler = async (ctx, projectId) => {
...
export const textMessageHandler = async (ctx) => {
    if (ctx.scene.session.step === ScraperSceneStep.ADD_COMPETITOR) {
        ...
        if (!ctx.message || !("text" in ctx.message) || !ctx.message.text) {
            await ctx.reply("Ошибка: пожалуйста, отправьте текстовое сообщение с URL Instagram-аккаунта");
            return;
        }
