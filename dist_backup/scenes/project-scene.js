 ...
 */
export const projectScene = new Scenes.BaseScene("instagram_scraper_projects");
...
projectScene.enter(async (ctx) => {
    try {
        ...
        if (!user) {
            await ctx.reply("Вы не зарегистрированы. Пожалуйста, используйте сначала основные команды бота.");
            await ctx.storage?.close();
            return await ctx.scene.leave();
        }
        const projects = await ctx.storage?.getProjectsByUserId(user.id);
        await ctx.reply(projects && projects.length > 0
            ? "Ваши проекты:"
        ...
        await ctx.reply("Произошла ошибка при получении проектов. Пожалуйста, попробуйте позже.");
        await ctx.storage?.close();
...
projectScene.action("create_project", async (ctx) => {
    ...
    await ctx.reply("Введите название нового проекта (например, 'Мой косметологический центр'):");
    ...
    if (ctx.scene.session.step === ScraperSceneStep.ADD_PROJECT) {
        try {
            ...
            if (projectName.length < 3) {
                await ctx.reply("Название проекта должно содержать не менее 3 символов. Попробуйте еще раз:");
                return;
            }
        ...
        await ctx.reply("Я не понимаю эту команду. Используйте кнопки для управления проектами.");
    }
});
...
projectScene.action(/project_(\d+)/, async (ctx) => {
    ...
    try {
        ...
        await ctx.reply("Произошла ошибка при получении данных проекта. Пожалуйста, попробуйте позже.");
        await ctx.storage?.close();
    }
});
