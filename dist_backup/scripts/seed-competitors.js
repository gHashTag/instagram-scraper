...
import { initializeNeonStorage, closeNeonStorage, createUser, getUserByTelegramId, createProject, getProjectsByUserId, addCompetitorAccount, getCompetitorAccounts, saveReels, scrapeInstagramReels, } from "../index";
...
async function main() {
    if (!DEMO_MODE && !APIFY_TOKEN) {
        ...
        console.log("Используйте --demo для запуска в демонстрационном режиме без API ключа");
        process.exit(1);
    }
    try {
        ...
        if (!user) {
            ...
            console.log(`Найден существующий пользователь: ${user.username} (ID: ${user.id})`);
        }
        ...
        if (!projects || projects.length === 0) {
            project = await createProject(user.id, "Aesthetic Medicine", "Проект по мониторингу конкурентов в нише эстетической медицины", "Эстетическая медицина");
            console.log(`Создан новый проект: ${project.name} (ID: ${project.id})`);
        }
        else {
            project = projects[0];
            console.log(`Найден существующий проект: ${project.name} (ID: ${project.id})`);
        }
        ...
        const existingUrls = existingCompetitorsList.map(comp => comp.instagram_url.toLowerCase());
        // Фильтруем и добавляем только новых конкурентов
        const competitorsToAdd = COMPETITOR_ACCOUNTS.filter(url => !existingUrls.includes(url.toLowerCase()));
        if (competitorsToAdd.length === 0) {
            ...
            for (const url of competitorsToAdd) {
                ...
                const competitor = await addCompetitorAccount(project.id, url, accountName);
        ...
        console.log("Результат getCompetitorAccounts:", JSON.stringify(competitors, null, 2).substring(0, 500));
        ...
        if (DEMO_MODE) {
            console.log(`ДЕМО-РЕЖИМ: Симуляция скрапинга для ${targetCompetitor.instagram_url}`);
            // Создаем демо-данные
            const demoReels = [
                {
                    ...
                    views_count: 75000,
                    likes_count: 1500,
                    description: "Демо-запись: Процедура для лица с использованием передовой технологии",
                    author_username: targetCompetitor.instagram_username ||
            ...
            console.log(`Сохранено ${demoReels.length} демо Reels в базу данных Neon.`);
        }
        else {
            ...
            try {
                ...
                // Запускаем скрапинг
                const reels = await scrapeInstagramReels(targetCompetitor.instagram_url, {
