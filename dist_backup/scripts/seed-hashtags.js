...
import { initializeNeonStorage, closeNeonStorage, getUserByTelegramId, getProjectsByUserId, addTrackingHashtag, getTrackingHashtags, saveReels, scrapeInstagramReels, } from "../index";
...
function generateDemoReels(hashtag) {
    const now = new Date();
    const demoReels = [];
    for (let i = 0; i < 5; i++) {
...
async function main() {
    ...
    try {
        ...
        if (!user) {
            console.error(`Пользователь с Telegram ID ${TELEGRAM_ID} не найден. Сначала запустите seed-competitors.ts для создания пользователя.`);
            process.exit(1);
        }
        ...
        if (!projects || projects.length === 0) {
            console.error(`Проекты для пользователя ID: ${user.id} не найдены. Сначала запустите seed-competitors.ts для создания проекта.`);
            process.exit(1);
        }
        ...
        const existingTags = existingHashtagsList.map(tag => tag.hashtag.toLowerCase());
        // Фильтруем и добавляем только новые хэштеги
        const hashtagsToAdd = HASHTAGS.filter(tag => !existingTags.includes(tag.toLowerCase()));
        if (hashtagsToAdd.length === 0) {
            ...
            for (const tag of hashtagsToAdd) {
                const hashtag = await addTrackingHashtag(project.id, tag, `#${tag}`, `Автоматически добавлен для мониторинга`);
        ...
        if (DEMO_MODE) {
            console.log(`ДЕМО-РЕЖИМ: Симуляция скрапинга для хэштега #${targetHashtag.hashtag}`);
            // Создаем демо-данные
            ...
            console.log(`Сохранено ${demoReels.length} демо Reels по хэштегу #${targetHashtag.hashtag} в базу данных Neon.`);
        }
        else {
            ...
            try {
                ...
                console.log(`Получено ${reels.length} Reels по хэштегу #${targetHashtag.hashtag}.`);
