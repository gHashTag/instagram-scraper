...
app.get("/", async (req, res) => {
    ...
    let counts = { users: 0, projects: 0, competitors: 0, hashtags: 0, reels: 0 };
    let testResults = {
    ...
    if (fs.existsSync(SQLITE_DB_PATH)) {
        try {
            ...
            for (const tableRow of tableRows) {
                ...
                // Update counts
                if (tableName === "Users")
                    counts.users = countQuery.count;
                if (tableName === "Projects")
                    counts.projects = countQuery.count;
                if (tableName === "Competitors")
                    counts.competitors = countQuery.count;
                if (tableName === "Hashtags")
                    counts.hashtags = countQuery.count;
                if (tableName === "ReelsContent")
                    counts.reels = countQuery.count;
            }
...
app.post("/api/reset-db", (req, res) => {
    try {
        ...
        exec(`cd ${ROOT_DIR} && RESET_DB=true bun run scripts/init-dev-db.ts`, (error, stdout, stderr) => {
            if (error) {
...
app.post("/api/generate-test-data", (req, res) => {
    try {
        // Run initialization script
        exec(`cd ${ROOT_DIR} && bun run scripts/init-dev-db.ts`, (error, stdout, stderr) => {
            if (error) {
...
app.post("/api/run-build", (req, res) => {
    try {
        // Run build script
        exec(`cd ${ROOT_DIR} && bun run scripts/build-and-test.sh`, (error, stdout, stderr) => {
            if (error) {
...
app.post("/api/run-unit-tests", (req, res) => {
    try {
        // Run unit tests
        exec(`cd ${ROOT_DIR} && bun vitest run __tests__/unit`, (error, stdout, stderr) => {
            if (error) {
...
app.post("/api/run-integration-tests", (req, res) => {
    try {
        ...
        exec(`cd ${ROOT_DIR} && bash scripts/test-integration.sh`, (error, stdout, stderr) => {
            if (error) {
...
// Start server
app.listen(PORT, () => {
    console.log(`✨ Instagram Scraper Bot - Панель Разработки запущена на http://localhost:${PORT}`);
    // Open in browser
