...
const SQLITE_DB_PATH = process.env.SQLITE_DB_PATH || path.resolve(DEV_DIR, "sqlite.db");
...
if (isDemoDataExists.count === 0 || process.env.NODE_ENV === "test") {
  ...
  `);
    ...
    VALUES (?, ?, ?)
  `);
    const projectResult = insertProject.run(userId, "Эстетическая косметология", "Проект для мониторинга конкурентов в сфере эстетической косметологии");
  ...
  `);
    ...
    for (const competitor of competitors) {
        insertCompetitor.run(projectId, competitor.username, competitor.fullName, competitor.profileUrl);
        competitorCount++;
    }
  ...
  `);
    ...
    for (const { id: competitorId } of competitorIds) {
        ...
        for (let i = 0; i < reelsToAdd; i++) {
            ...
            insertReel.run(projectId, "competitor", competitorId, reelId, `https://instagram.com/reel/${reelId}/`, "Демо-контент для тестирования. #красота #косметология #тест", `competitor_${competitorId}`, `owner_${competitorId}`, viewCount, likeCount, commentCount, Math.floor(Math.random() * 45) + 15, // 15-60 сек
            `https://example.com/thumbnails/${reelId}.jpg`, `Демо-аудио ${Math.floor(Math.random() * 100)}`, randomDate());
            reelsCount++;
        }
    }
    ...
    for (const { id: hashtagId } of hashtagIds) {
        ...
        for (let i = 0; i < reelsToAdd; i++) {
            ...
            insertReel.run(projectId, "hashtag", hashtagId, reelId, `https://instagram.com/reel/${reelId}/`, "Демо-контент по хэштегу для тестирования. #красота #косметология #тест", `random_user_${Math.floor(Math.random() * 1000)}`, `random_owner_${Math.floor(Math.random() * 10000)}`, viewCount, likeCount, commentCount, Math.floor(Math.random() * 45) + 15, // 15-60 сек
            `https://example.com/thumbnails/${reelId}.jpg`, `Демо-аудио хэштег ${Math.floor(Math.random() * 100)}`, randomDate());
            reelsCount++;
        }
    }
